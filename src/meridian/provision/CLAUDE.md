...
- **`return 444` is banned from HTTPS blocks** — blind censor assessment (three independent experts) rated 444 (silent close after TLS handshake) as 9/10 suspicious — virtually no legitimate server does this. Root now serves a 200 placeholder page ("under construction"), rated 2/10. Other paths return 404. The `--decoy` flag is deprecated.
