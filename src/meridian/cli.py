...
def _server(ctx: typer.Context) -> str:
    """Get the --server value from context."""
    return (ctx.obj or {}).get("server", "")
...
def setup_cmd(
    ...
    run(ip, domain, email, sni, xhttp, name, user, yes, _server(ctx))
...
def client_remove(
    ...
    run_remove(name, user, _server(ctx))
...
def check_cmd(
    ...
    run(ip, domain, sni, user, ai, _server(ctx))
...
def diagnostics_cmd(
    ...
    run(ip, sni, user, ai, _server(ctx))
