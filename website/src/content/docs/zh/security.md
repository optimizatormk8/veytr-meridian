...
- **Panel 访问**：在所有模式中由 Caddy 在秘密 HTTPS 路径上反向代理——无需 SSH 隧道
- **SSH**：默认禁用密码认证
- **防火墙**：UFW 配置为默认拒绝，仅打开端口 22、80 和 443
- **Docker**：3x-ui 镜像固定到已测试的版本
- **TLS**：Caddy 通过 Let's Encrypt 自动处理证书
...
- [Caddy](https://github.com/caddyserver/caddy)——TLS 终止
- [HAProxy](https://www.haproxy.org/)——SNI 路由
